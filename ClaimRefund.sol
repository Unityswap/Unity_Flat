...
// File: contracts/ClaimBackRyip.sol

pragma solidity ^0.6.12;

contract ClaimBackRyip is Ownable {
    using SafeBEP20 for IBEP20;

    IBEP20 public ryipToken;
    UnityProfile unityProfile;

    uint256 public numberRyip;
    ...
    constructor(
        ...
        ryipToken = _ryipToken;
        unityProfile = UnityProfile(_unityProfileAddress);
        numberRyip = _numberRyip;
        thresholdUser = _thresholdUser;
    }

    function getRyipBack() external {
        // 1. Check if she has registered
        require(unityProfile.hasRegistered(_msgSender()), "not active");

        // 2. Check if she has claimed
        require(!hasClaimed[_msgSender()], "has claimed RYIP");

        // 3. Check if she is active
        uint256 userId;
        (userId, , , , , ) = unityProfile.getUserProfile(_msgSender());
        ...
        // Transfer RYIP tokens from this contract
        ryipToken.safeTransfer(_msgSender(), numberRyip);
    }
