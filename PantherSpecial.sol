...
// File: @unityswap/unity-swap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/UnityPanthers.sol

pragma solidity ^0.6.0;

contract UnityPanthers is ERC721, Ownable {
    using Counters for Counters.Counter;

    // Map the number of tokens per pantherId
    mapping(uint8 => uint256) public pantherCount;

    // Map the number of tokens burnt per pantherId
    mapping(uint8 => uint256) public pantherBurnCount;

    // Used for generating the tokenId of new NFT minted
    Counters.Counter private _tokenIds;

    // Map the pantherId for each tokenId
    mapping(uint256 => uint8) private pantherIds;

    // Map the pantherName for a tokenId
    mapping(uint8 => string) private pantherNames;

    constructor(string memory _baseURI) public ERC721("Unity Panthers", "PB") {
        _setBaseURI(_baseURI);
    }

    /**
     * @dev Get pantherId for a specific tokenId.
     */
    function getPantherId(uint256 _tokenId) external view returns (uint8) {
        return pantherIds[_tokenId];
    }

    /**
     * @dev Get the associated pantherName for a specific pantherId.
     */
    function getPantherName(uint8 _pantherId)
        external
        view
        returns (string memory)
    {
        return pantherNames[_pantherId];
    }

    /**
     * @dev Get the associated pantherName for a unique tokenId.
     */
    function getPantherNameOfTokenId(uint256 _tokenId)
        external
        view
        returns (string memory)
    {
        uint8 pantherId = pantherIds[_tokenId];
        return pantherNames[pantherId];
    }
    ...
    function mint(
        ...
        _tokenIds.increment();
        pantherIds[newId] = _pantherId;
        pantherCount[_pantherId] = pantherCount[_pantherId].add(1);
        _mint(_to, newId);
    ...
    /**
     * @dev Set a unique name for each pantherId. It is supposed to be called once.
     */
    function setPantherName(uint8 _pantherId, string calldata _name)
        external
        onlyOwner
    {
        pantherNames[_pantherId] = _name;
    }
    ...
    function burn(uint256 _tokenId) external onlyOwner {
        uint8 pantherIdBurnt = pantherIds[_tokenId];
        pantherCount[pantherIdBurnt] = pantherCount[pantherIdBurnt].sub(1);
        pantherBurnCount[pantherIdBurnt] = pantherBurnCount[pantherIdBurnt].add(1);
        _burn(_tokenId);
    }
}

// File: contracts/PantherMintingStation.sol

pragma solidity ^0.6.0;

/** @title PantherMintingStation.
@dev It is a contract that allow different factories to mint
Unity Collectibles/Panthers.
*/

contract PantherMintingStation is AccessControl {
    UnityPanthers public unityPanthers;
    ...
    constructor(UnityPanthers _unityPanthers) public {
        unityPanthers = _unityPanthers;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev Mint NFTs from the UnityPanthers contract.
     * Users can specify what pantherId they want to mint. Users can claim once.
     * There is a limit on how many are distributed. It requires RYIP balance to be > 0.
     */
    function mintCollectible(
        ...
        uint256 tokenId =
            unityPanthers.mint(_tokenReceiver, _tokenURI, _pantherId);
        return tokenId;
    }

    /**
     * @dev Set up names for panthers.
     * Only the main admins can set it.
     */
    function setPantherName(uint8 _pantherId, string calldata _pantherName)
        external
        onlyOwner
    {
        unityPanthers.setPantherName(_pantherId, _pantherName);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        unityPanthers.transferOwnership(_newOwner);
    }
}
...
// File: contracts/utils/UnityProfile.sol
...
*/
contract UnityProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public ryipToken;
    ...
    uint256 public numberRyipToReactivate;
    uint256 public numberRyipToRegister;
    uint256 public numberRyipToUpdate;
    ...
    constructor(
        IBEP20 _ryipToken,
        uint256 _numberRyipToReactivate,
        uint256 _numberRyipToRegister,
        uint256 _numberRyipToUpdate
    ) public {
        ryipToken = _ryipToken;
        numberRyipToReactivate = _numberRyipToReactivate;
        numberRyipToRegister = _numberRyipToRegister;
        numberRyipToUpdate = _numberRyipToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends RYIP to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer RYIP tokens to this contract
        ryipToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberRyipToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer RYIP token to this address
        ryipToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberRyipToUpdate
    ...
    /**
     * @dev Claim RYIP to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        ryipToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of RYIP to register
     * Callable only by owner admins.
     */
    function updateNumberRyip(
        uint256 _newNumberRyipToReactivate,
        uint256 _newNumberRyipToRegister,
        uint256 _newNumberRyipToUpdate
    ) external onlyOwner {
        numberRyipToReactivate = _newNumberRyipToReactivate;
        numberRyipToRegister = _newNumberRyipToRegister;
        numberRyipToUpdate = _newNumberRyipToUpdate;
    }
...
// File: contracts/PantherSpecialV1.sol

pragma solidity ^0.6.12;

contract PantherSpecialV1 is Ownable {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    PantherMintingStation public pantherMintingStation;
    UnityProfile public unityProfile;

    IBEP20 public ryipToken;

    uint256 public maxViewLength;
    uint256 public numberDifferentPanthers;

    // Map if address for a pantherId has already claimed a NFT
    mapping(address => mapping(uint8 => bool)) public hasClaimed;

    // Map if pantherId to its characteristics
    mapping(uint8 => Panthers) public pantherCharacteristics;

    // Number of previous series (i.e. different visuals)
    uint8 private constant previousNumberPantherIds = 10;
    ...
    // Event to notify a new panther is mintable
    event PantherAdd(
        uint8 indexed pantherId,
        uint256 thresholdUser,
        uint256 costRyip
    );

    // Event to notify one of the panthers' requirements to mint differ
    event PantherChange(
        uint8 indexed pantherId,
        uint256 thresholdUser,
        uint256 costRyip,
    ...
    constructor(
        PantherMintingStation _pantherMintingStation,
        IBEP20 _ryipToken,
        UnityProfile _unityProfile,
    ...
    /**
     * @dev Mint NFTs from the PantherMintingStation contract.
     * Users can claim once.
     */
    function mintNFT(uint8 _pantherId) external {
        // Check that the _pantherId is within boundary
        require(_pantherId >= previousNumberPantherIds, "ERR_ID_LOW");
        require(pantherCharacteristics[_pantherId].isActive, "ERR_ID_INVALID");

        address senderAddress = _msgSender();

        // 1. Check _msgSender() has not claimed
        require(!hasClaimed[senderAddress][_pantherId], "ERR_HAS_CLAIMED");

        uint256 userId;
        bool isUserActive;

        (userId, , , , , isUserActive) = unityProfile.getUserProfile(
            senderAddress
        );

        require(
            userId < pantherCharacteristics[_pantherId].thresholdUser,
            "ERR_USER_NOT_ELIGIBLE"
        );

        require(isUserActive, "ERR_USER_NOT_ACTIVE");

        // Check if there is any cost associated with getting the panther
        if (pantherCharacteristics[_pantherId].ryipCost > 0) {
            ryipToken.safeTransferFrom(
                senderAddress,
                address(this),
                pantherCharacteristics[_pantherId].ryipCost
            );
        }

        // Update that _msgSender() has claimed
        hasClaimed[senderAddress][_pantherId] = true;

        uint256 tokenId =
            pantherMintingStation.mintCollectible(
                senderAddress,
                pantherCharacteristics[_pantherId].tokenURI,
                _pantherId
            );

        emit PantherMint(senderAddress, tokenId, _pantherId);
    }
    ...
    function addPanther(
        ...
        require(!pantherCharacteristics[_pantherId].isCreated, "ERR_CREATED");
        require(_pantherId >= previousNumberPantherIds, "ERR_ID_LOW_2");

        pantherCharacteristics[_pantherId] = Panthers({
            tokenURI: _tokenURI,
        ...
        numberDifferentPanthers = numberDifferentPanthers.add(1);

        emit PantherAdd(_pantherId, _thresholdUser, _ryipCost);
    }

    /**
     * @dev It transfers the RYIP tokens back to the chef address.
    ...
    function updatePanther(
        uint8 _pantherId,
        uint256 _thresholdUser,
        uint256 _ryipCost,
        bool _isActive
    ) external onlyOwner {
        require(pantherCharacteristics[_pantherId].isCreated, "ERR_NOT_CREATED");
        pantherCharacteristics[_pantherId].thresholdUser = _thresholdUser;
        pantherCharacteristics[_pantherId].ryipCost = _ryipCost;
        pantherCharacteristics[_pantherId].isActive = _isActive;

        emit PantherChange(_pantherId, _thresholdUser, _ryipCost, _isActive);
    }
    ...
    function canClaimSingle(address _userAddress, uint8 _pantherId)
        external
        view
        returns (bool)
    {
        if (!unityProfile.hasRegistered(_userAddress)) {
            return false;
        } else {
            uint256 userId;
            bool userStatus;

            (userId, , , , , userStatus) = unityProfile.getUserProfile(
                _userAddress
            );
            ...
            if (!userStatus) {
                ...
                bool claimStatus = _canClaim(_userAddress, userId, _pantherId);
                return claimStatus;
            }
        }
    }

    function canClaimMultiple(address _userAddress, uint8[] calldata _pantherIds)
        external
        view
        returns (bool[] memory)
    {
        require(_pantherIds.length <= maxViewLength, "ERR_LENGTH_VIEW");
        ...
        if (!unityProfile.hasRegistered(_userAddress)) {
            ...
            if (!userStatus) {
                ...
                bool[] memory responses = new bool[](_pantherIds.length);

                for (uint256 i = 0; i < _pantherIds.length; i++) {
                    bool claimStatus =
                        _canClaim(_userAddress, userId, _pantherIds[i]);
    ...
    function _canClaim(
        ...
        uint256 pantherThreshold = pantherCharacteristics[_pantherId].thresholdUser;
        bool pantherActive = pantherCharacteristics[_pantherId].isActive;

        if (hasClaimed[_userAddress][_pantherId]) {
            return false;
        } else if (!pantherActive) {
            return false;
        } else if (userId >= pantherThreshold) {
            return false;
        } else {
            return true;
        }
