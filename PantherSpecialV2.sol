...
/** @title UnityPanthers.
 * @notice It is the contracts for UnitySwap NFTs.
 */
contract UnityPanthers is ERC721, Ownable {
   ...
   */
  function getPantherName(uint8 _pantherId) external view returns (string memory) {
...
contract PantherMintingStation is AccessControl {
  ...
  /**
   * @notice Mint NFTs from the UnityPanthers contract.
  ...
  function mintCollectible(
    ...
    uint256 tokenId = unityPanthers.mint(_tokenReceiver, _tokenURI, _pantherId);
    return tokenId;
  }

  /**
   * @notice Set up names for panthers.
...
// File: contracts/UnityProfile.sol
...
contract UnityProfile is AccessControl, ERC721Holder {
  ...
  function updateProfile(address _nftAddress, uint256 _tokenId) external {
    ...
    ryipToken.safeTransferFrom(_msgSender(), address(this), numberRyipToUpdate);
...
// File: contracts/PantherSpecialV2.sol
 ...
 */
contract PantherSpecialV2 is Ownable {
  using SafeBEP20 for IBEP20;
  using SafeMath for uint256;
  ...
  uint8 public constant previousNumberPantherIds = 12;

  uint256 public endBlock;
  uint256 public thresholdUser;

  // Map if pantherId to its tokenURI
  mapping(uint8 => string) public pantherTokenURI;

  // Map if address has already claimed a NFT
  mapping(address => bool) public hasClaimed;

  // Map teamId to its pantherId
  mapping(uint256 => uint8) public teamIdToPantherId;

  event PantherAdd(uint8 pantherId, uint256 teamId);
  ...
  function mintNFT() external {
    ...
    (userId, , userTeamId, , , isUserActive) = unityProfile.getUserProfile(
      senderAddress
    );
    ...
    uint8 pantherId = teamIdToPantherId[userTeamId];

    require(pantherId >= previousNumberPantherIds, "NOT_VALID");

    string memory tokenURI = pantherTokenURI[pantherId];

    uint256 tokenId =
      pantherMintingStation.mintCollectible(senderAddress, tokenURI, pantherId);

    emit PantherMint(senderAddress, tokenId, pantherId);
  }
  ...
  function addPanther(
    ...
    teamIdToPantherId[_teamId] = _pantherId;
    pantherTokenURI[_pantherId] = _tokenURI;

    emit PantherAdd(_pantherId, _teamId);
  }
  ...
  function canClaim(address _userAddress) external view returns (bool) {
    if (hasClaimed[_userAddress]) {
      ...
      if (!unityProfile.getUserStatus(_userAddress)) {
        return false;
      } else {
        uint256 userId;
        (userId, , , , , ) = unityProfile.getUserProfile(_userAddress);
