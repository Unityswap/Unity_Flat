...
// File: contracts/interfaces/IUnityProfile.sol

pragma solidity ^0.6.0;

/** @title IUnityProfile.
 */
interface IUnityProfile {
     ...
     * Callable only by owner admins.
     */
    function updateNumberRyip() external;
...
contract UnityPanthers is ERC721, Ownable {
     ...
     */
    function getPantherNameOfTokenId(uint256 _tokenId) external view returns (string memory) {
     ...
     */
    function setPantherName(uint8 _pantherId, string calldata _name) external onlyOwner {
...
contract PantherMintingStation is AccessControl {
     ...
     */
    function setPantherName(uint8 _pantherId, string calldata _pantherName) external onlyOwner {
...
contract TradingCompV1 is Ownable {
    ...
    IUnityProfile public unityProfile;

    uint256 public constant numberTeams = 3;

    uint8 public pantherId;
    ...
    struct CompetitionRewards {
        ...
        uint256[5] ryipRewards; // ryip rewards per group
     ...
     * @param _unityProfileAddress: UnityProfile address
     * @param _pantherStationAddress: PantherMintingStation address
     * @param _ryipTokenAddress: the address of the RYIP token
     */
    constructor(
        address _unityProfileAddress,
        address _pantherStationAddress,
        address _ryipTokenAddress
    ) public {
        unityProfile = IUnityProfile(_unityProfileAddress);
        pantherMintingStation = PantherMintingStation(_pantherStationAddress);
        ryipToken = IBEP20(_ryipTokenAddress);
    ...
    function claimReward() external {
        ...
        if (userRewardGroup > 0) {
            ryipToken.safeTransfer(senderAddress, userRewards.ryipRewards[userRewardGroup]);
        ...
        unityProfile.increaseUserPoints(
            senderAddress,
     ...
     * @dev Only callable if the user has an active UnityProfile.
     */
    function register() external {
        ...
        (, , userTeamId, , , isUserActive) = unityProfile.getUserProfile(senderAddress);
    ...
    /**
     * @notice It allows the owner to claim the RYIP remainder
     * @dev Only callable by owner.
     * @param _amount: amount of RYIP to withdraw (decimals = 18)
     */
     ...
     * @param _ryipRewards: RYIP rewards for each user group for teamId
     ...
     * @param _pantherId: the pantherId for winners (e.g. 15)
     */
    function updateWinningTeamAndTokenURIAndPantherId(
        ...
        require(_pantherId > 14, "ID_TOO_LOW");
        winningTeamId = _winningTeamId;
        tokenURI = _tokenURI;
        pantherId = _pantherId;
     ...
     * @return userRyipRewards: the RYIP to claim/claimed
        ...
        )
    {
        ...
        if ((currentStatus != CompetitionStatus.Claiming) && (currentStatus != CompetitionStatus.Over)) {
            ...
            uint256 userRyipRewards = _rewardCompetitions[userTeamId].ryipRewards[userRewardGroup];
            ...
            return (hasUserRegistered, hasUserClaimed, userRewardGroup, userRyipRewards, userPointRewards, canClaimNFT);
        }
    }
